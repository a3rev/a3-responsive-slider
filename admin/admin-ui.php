...
class A3_Responsive_Slider_Admin_UI
{
	...
	public $google_api_key_option = 'a3_responsive_slider_google_api_key';

	public $toggle_box_open_option = 'a3_responsive_slider_toggle_box_open';

	public $is_free_plugin = true;

	public $version_transient = 'a3_responsive_slider_licinfo';

	public $plugin_option_key = 'a3rev_rslider_plugin';

	public $support_url = 'https://wordpress.org/support/plugin/a3-responsive-slider/';
	 ...
	 */
	public $pro_plugin_page_url = 'http://a3rev.com/shop/a3-responsive-slider/';
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				...
				if ( FALSE !== stristr( $version_transient, '||' )
					...
					&& version_compare( get_option('a3rev_rslider_version') , $version_info[0], '<' ) ) {
						...
						$version_message = sprintf( __( 'There is a new version <span class="a3rev-ui-new-plugin-version">%s</span> available, <a href="%s" target="_blank">update now</a> or download direct from <a href="%s" target="_blank">My Account</a> on a3rev.com', 'a3-responsive-slider' ),
							$version_info[0],
							wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' . A3_RESPONSIVE_SLIDER_NAME ), 'upgrade-plugin_' . A3_RESPONSIVE_SLIDER_NAME ),
