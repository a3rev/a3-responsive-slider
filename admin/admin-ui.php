...
class Admin_UI
{
	...
	public $plugin_path            = A3_RESPONSIVE_SLIDER_NAME;
	...
	public $plugin_prefix = A3_RESPONSIVE_SLIDER_PREFIX;

	/**
	 * @var string
	 * You must change to correct pro plugin page url on a3rev site
	 */
	public $pro_plugin_page_url = 'https://a3rev.com/shop/responsive-slider/';
	 ...
	 */
	public $admin_pages = array( 'a3-responsive-slider', 'a3-rslider-add' );

	public function __construct() {
		$this->google_api_key_option     = A3_RESPONSIVE_SLIDER_KEY . '_google_api_key';
		$this->google_map_api_key_option = A3_RESPONSIVE_SLIDER_KEY . '_google_map_api_key';
		$this->toggle_box_open_option    = A3_RESPONSIVE_SLIDER_KEY . '_toggle_box_open';
		$this->version_transient         = A3_RESPONSIVE_SLIDER_KEY . '_licinfo';

		if ( defined( 'A3_RESPONSIVE_SLIDER_G_FONTS' ) ) {
			$this->is_load_google_fonts = (boolean) A3_RESPONSIVE_SLIDER_G_FONTS;
		}

		$this->support_url = 'https://wordpress.org/support/plugin/a3-responsive-slider/';
	}
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				'box_title'    => __( 'Premium Version Enhanced Features', 'a3-responsive-slider' ),
				'image_url'    => A3_RESPONSIVE_SLIDER_IMAGES_URL. '/video.jpg',
				'video_url'    => 'https://www.youtube.com/embed/9dGw-ORfMIk?version=3&autoplay=1',
				'left_title'   => __( 'Premium Version Enhanced Features', 'a3-responsive-slider' ),
				'left_text'    => __( 'a3 Responsi Slider Premium', 'a3-responsive-slider' )
									. "\n\n" . __( 'Quick Video showing the main (not all) enhanced features that are built into the a3 Responsi Slider Premium version', 'a3-responsive-slider' ),
				'right_title'  => __( 'Developer Support and Premium Features', 'a3-responsive-slider' ),
				'right_text'   => __( 'Limited Time Offer. Purchase the Premium Version Lifetime License. That is a Lifetime of maintenance updates, feature upgrades and developer support for a once only fee. Offer ending soon.', 'a3-responsive-slider' )
									. "\n\n" . '<a target="_blank" href="'.$this->pro_plugin_page_url.'" class="button-primary">' . __( 'Get Premium Features and Support', 'a3-responsive-slider' ) . '</a>',
			);
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">' 
			. __( 'Advanced Settings - Upgrade to the <a href="%s" target="_blank">%s License</a> to activate these settings.', 'a3-responsive-slider' ) 
			. '</div>'
			...
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Pro Version', 'a3-responsive-slider' ) ) )
		);
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'a3-responsive-slider' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'a3-responsive-slider' ), $this->support_url );
			} else {
				...
				if ( FALSE !== stristr( $version_transient, '||' )
					...
					&& version_compare( A3_RESPONSIVE_SLIDER_VERSION , $version_info[0], '<' ) ) {

						$version_message = sprintf( __( 'There is a new version <span class="a3rev-ui-new-plugin-version">%s</span> available, <a href="%s" target="_blank">update now</a> or download direct from <a href="%s" target="_blank">My Account</a> on a3rev.com', 'a3-responsive-slider' ),
							$version_info[0],
